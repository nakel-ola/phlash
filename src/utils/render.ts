...
function _render<TTag extends ElementType, TSlot>(
  ...
  if ((rest as any).className && typeof (rest as any).className === 'function') {
    ;(rest as any).className = (rest as any).className(slot)
  }
    ...
    if (Object.keys(compact(rest)).length > 0) {
      ...
      return cloneElement(
        ...
        Object.assign(
          ...
          mergeProps((resolvedChildren as any).props, compact(omit(rest, ['ref']))),
          dataAttributes,
          refRelatedProps
        )
      )
...
function omit<T extends Record<any, any>>(object: T, keysToOmit: string[] = []) {
  let clone = Object.assign({}, object) as any;
